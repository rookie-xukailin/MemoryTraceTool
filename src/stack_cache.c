...
/* 指纹索引表:按 fp 哈希分桶,BSS 零初始化。
 * 与主栈缓存独立,业务线程(malloc 热路径)查询,reporter 线程也访问,
 * 用独立锁保护避免与主缓存 lock 交织 */
static mtt_fp_entry_t *g_fp_cache[MTT_FP_CACHE_SIZE];
static pthread_mutex_t g_fp_cache_lock = PTHREAD_MUTEX_INITIALIZER;

int mtt_fp_cache_lookup(uint64_t fp, void **frames, int max_frames)
{
    if (fp == 0 || frames == NULL || max_frames <= 0) return 0;

    unsigned bucket = (unsigned)(fp & (uint64_t)(MTT_FP_CACHE_SIZE - 1));

    pthread_mutex_lock(&g_fp_cache_lock);
    mtt_fp_entry_t *e = g_fp_cache[bucket];
    while (e != NULL) {
        if (e->fp == fp) {
            int n = e->frame_count;
            if (n > max_frames) n = max_frames;
            memcpy(frames, e->frames, (size_t)n * sizeof(void*));
            pthread_mutex_unlock(&g_fp_cache_lock);
            return n;
        }
        e = e->next;
    }
...
{
    if (fp == 0 || frames == NULL || frame_count <= 0) return;
    if (frame_count > MTT_STACK_DEPTH) frame_count = MTT_STACK_DEPTH;
    ...
    /* 新条目(用 raw_malloc 避免 hook 重入;失败静默跳过,不影响正确性) */
    mtt_fp_entry_t *ne = (mtt_fp_entry_t*)raw_malloc(sizeof(mtt_fp_entry_t));
    if (ne != NULL) {
        memset(ne, 0, sizeof(*ne));
        ne->fp          = fp;
        ne->frame_count = frame_count;
        memcpy(ne->frames, frames, (size_t)frame_count * sizeof(void*));
        ne->next = g_fp_cache[bucket];
        g_fp_cache[bucket] = ne;
    }

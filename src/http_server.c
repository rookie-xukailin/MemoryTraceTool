...
"function fy(b){if(b==null||b<0)b=0;if(yUnit==='b')return b.toLocaleString()+' B';if(yUnit==='mb')return b>=1048576?(b/1048576).toFixed(2)+' MB':(b/1024).toFixed(1)+' KB';return b>=1024?Math.round(b/1024).toLocaleString()+' KB':b.toLocaleString()+' B'}\n"
...
"  /* peak line: 虚线,超出当前 Y 轴时 clamp 到顶(不再撑大 Y 轴) */\n"
"  function yc(v){return Math.max(pad.top,y(v))}\n"

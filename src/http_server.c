...
"<div class=\"card\">\n"
"  <h2>堆内存趋势 <span class=\"unit-toggle\" id=\"unitToggle\"><button class=\"unit-btn\" data-unit=\"b\">B</button><button class=\"unit-btn\" data-unit=\"kb\">KB</button><button class=\"unit-btn\" data-unit=\"mb\">MB</button></span><span class=\"refresh\" id=\"refreshLabel\">每 5 秒刷新</span></h2>\n"
...
"var yUnit=(function(){try{return localStorage.getItem('mtt_yunit')||'kb'}catch(e){return'kb'}})();\n"
"function fy(b){if(b==null||b<0)b=0;if(yUnit==='b')return b.toLocaleString()+' B';if(yUnit==='mb')return(b/1048576).toFixed(2)+' MB';return Math.round(b/1024).toLocaleString()+' KB'}\n"
...
"  var maxB=1;\n"
"  for(var i=0;i<ts.length;i++){var p=ts[i];if(p.peak>maxB)maxB=p.peak;if(p.cur>maxB)maxB=p.cur;if(p.leak!=null&&p.leak>maxB)maxB=p.leak;}\n"
"  if(maxB<1)maxB=1;\n"
...
"  for(var j=0;j<=4;j++){var val=(maxB/4)*j,yy=y(val);ctx.beginPath();ctx.moveTo(pad.left,yy);ctx.lineTo(W-pad.right,yy);ctx.stroke()}\n"
"  ctx.globalAlpha=1;\n"
"  /* Y labels: 按 yUnit(B/KB/MB)切换 */\n"
"  ctx.fillStyle=txtColor;ctx.globalAlpha=0.65;ctx.font='10px -apple-system,BlinkMacSystemFont,sans-serif';ctx.textAlign='right';\n"
"  for(var j=0;j<=4;j++){var val=(maxB/4)*j,yy=y(val);ctx.fillText(fy(val),pad.left-8,yy+3)}\n"
"  ctx.globalAlpha=1;\n"
"  /* area fill: 渐变 + 平滑曲线 */\n"
"  if(ts.length>1){\n"
...
"    ctx.lineTo(x(0),y(ts[0].cur));\n"
"    for(var i=1;i<ts.length;i++){var mx=(x(i-1)+x(i))/2,my=(y(ts[i-1].cur)+y(ts[i].cur))/2;ctx.quadraticCurveTo(x(i-1),y(ts[i-1].cur),mx,my)}\n"
"    ctx.lineTo(x(ts.length-1),y(ts[ts.length-1].cur));\n"
...
"    ctx.moveTo(x(0),y(ts[0].leak));\n"
"    for(var i=1;i<ts.length;i++){if(ts[i].leak==null)continue;var mx=(x(i-1)+x(i))/2,my=(y(ts[i-1].leak)+y(ts[i].leak))/2;ctx.quadraticCurveTo(x(i-1),y(ts[i-1].leak),mx,my)}\n"
"    ctx.lineTo(x(ts.length-1),y(ts[ts.length-1].leak));\n"
"    ctx.strokeStyle='#dc2626';ctx.lineWidth=2;ctx.lineJoin='round';ctx.lineCap='round';ctx.stroke();\n"
"  }\n"
"  /* legend: 圆点 + 文字 */\n"
